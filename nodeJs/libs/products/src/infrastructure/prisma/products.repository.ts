...
export class ProductsRepository implements IProductsRepository {
  ...
  async findById(id: string): Promise<Result<Product, BasicError>> {
    try {
      ...
      return basicErr(`Failed to find product: ${getErrorMessage(error)}`);
    }
  }
  ...
  async create(
    ...
    try {
      ...
      return basicErr(`Failed to create product: ${getErrorMessage(error)}`);
    }
  }
  ...
  async getPaged(
    ...
    try {
      ...
      return basicErr(`Failed to paginate products: ${getErrorMessage(error)}`);
    }
  }
}
